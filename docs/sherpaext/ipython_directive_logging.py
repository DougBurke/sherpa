# -*- coding: utf-8 -*-
"""
This is a copy of https://github.com/ipython/ipython/blob/master/IPython/sphinxext/ipython_directive.py 

It has been extended to allow an operation to be performed on the
stdout stream used by the embedded IPython interpreter. I needed this
to add a logging handler to add output to this stream, but it may be
useful for other options (and done this way minimises the imppact on
the existing code).
...
ipython_process_stream
    If not `None` then it should be a function which accepts one
    values, the stdout stream used by the embedded IPython interpreter.
    This function is called after the embedded shell is created.
...
# Try to support old and new versions
try:
...
class IPythonDirective(Directive):
    ...
    def setup(self):
        ...
        if self.shell is None:
            ...
            # Do not access this via get_config_options to reduce the
            # changes. Should add_logger be sent to the shell itself?
            process_stream = self.state.document.settings.env.config.ipython_process_stream
            if process_stream is not None:
                process_stream(self.shell.cout)
...
def setup(app):
    ...
    app.add_config_value('ipython_process_stream', None, 'env')
