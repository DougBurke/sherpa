...
def test_regrid_table_requires_bins(make_data_path):
    """The regrid method requires lo,hi bins for tables"""
...
def test_combined(name1, par1, val1, name2, par2, val2):
    ...
    com1 = getattr(xspec, 'XS{}'.format(name1))('com1')
    com2 = getattr(xspec, 'XS{}'.format(name2))('com2')
...
@pytest.mark.parametrize("name,par,val",
                         [('wabs', 'nh', 0.05),
                          ('powerlaw', 'norm', 100)])
def test_combined_arithmetic_left(name, par, val):
    """Simple test of an additive model * multiplicative model"""
    ...
    com = getattr(xspec, 'XS{}'.format(name))('com')
    setattr(com, par, val)
...
@pytest.mark.parametrize("name,par,val",
                         [('wabs', 'nh', 0.05),
                          ('powerlaw', 'norm', 100)])
def test_combined_arithmetic_right(name, par, val):
    """Simple test of an additive model * multiplicative model"""
...
@requires_xspec
@pytest.mark.parametrize("mname", ["wabs", "powerlaw"])
def test_regrid_convolution_requires_bins(mname):
    """The regrid method requires lo,hi bins"""
...
def test_regrid_convolution_single(model, tol):
    ...
    mdl = getattr(xspec, 'XS{}'.format(model))()
    cmdl = conv(mdl)
...
@pytest.mark.parametrize('name1,name2',
                         [('wabs', 'powerlaw'),
                          ('powerlaw', 'wabs')])
def test_regrid_convolution_combined(name1, name2):
    ...
    mdl1 = getattr(xspec, 'XS{}'.format(name1))()
    mdl2 = getattr(xspec, 'XS{}'.format(name2))()
    cmdl = conv(mdl1 * mdl2)
