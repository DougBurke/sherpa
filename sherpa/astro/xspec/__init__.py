...
def set_xsversion(name: VersionType,
    ...
    if name == "atomdb":
        _xspec.set_xsversion_atomdb(version)
        got = _xspec.get_xsversion_atomdb()
        xsstate["versions"]["atomdb"] = got
        return

    if name == "nei":
        _xspec.set_xsversion_nei(version)
        got = _xspec.get_xsversion_nei()
        xsstate["versions"]["nei"] = got
        return
...
def set_xsstate(state: dict[str, Any]) -> None:
    ...
    # abund is only set if abundances is not set.
    #
