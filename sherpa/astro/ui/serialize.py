...
def _load_from_store(out: OutType,
    ...
    If the return value is False then the dataset needs to be manually
    created.
...
def _save_dataset_settings_pha(out: OutType,
    ...
    if len(rids) > 0:
        ...
        for rid in rids:
            ...
            _load_arf(out, arf, idval, rid, store=arf_store,
                      auto_load=auto_load)
            _load_rmf(out, rmf, idval, rid, store=rmf_store,
                      auto_load=auto_load)
        ...
        for bid in bids:
            ...
            if len(rids) > 0:
                ...
                for rid in rids:
                    ...
                    _load_arf(out, bkg_arf, idval, rid,
                              store=bkg_arf_store_bid,
                              auto_load=auto_load, bid=bid)
                    _load_rmf(out, bkg_rmf, idval, rid,
                              store=bkg_rmf_store_bid,
                              auto_load=auto_load, bid=bid)
