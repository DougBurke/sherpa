...
def setup_linked_pars():
    """Create simple setup for test_normal_sample_linked_par_xxx"""

    # Fit the same data with the same model, but the second one uses a
    # linked parameter for the c0 parameter.
    #
...
def basic_linked_par_validation(mdl1, mdl2, f1, f2, c1, c2) -> None:
    ...
    # The error results are "mdl1.c0, mdl1.c1" for dataset 1 and
    # "mdl2.c1, linkedpar.c0" for dataset 2, thanks to how we handle
    # linker parameters. This is possibly not ideal.
    #
...
def test_normal_sample_linked_par_chisq(correlate, clean_ui):
    ...
    Check with a chi-square statistic even though the data is really
    poisson.
...
def test_normal_sample_linked_par_cash(correlate, clean_ui):
    ...
    Try with a more-relevant statistic and method.
    ...
    # Unlike test_normal_sample_linked_par_chisq, the statistic
    # values are not the same for the two data sets.
    #
    assert s2[:, 0] != pytest.approx(s1[:, 0])
    ...
    assert check1 == pytest.approx(s1[0, 0])
    assert check2 == pytest.approx(s2[0, 0])

    # This location should be worse than the best-fit location.
    #
    assert check1 > f1.statval
    assert check2 > f2.statval
